 ...
 * @notice Refactor of MerkleTreeWithHistory where ALL tree state is namespaced by
 *         `uint256 assetId`. This lets a single contract host one independent
 *         incremental Merkle tree per asset (per-asset anonymity set) while reusing
 *         the exact same Poseidon-2 hasher and zero values as the original single-asset
 *         tree — so the compiled `transaction2` circuit/zkey is reused with NO recompile.
 *
 *         A tree must be initialized for an asset via `_initTree(assetId)` before use.
 */
contract MultiAssetMerkleTree is Initializable {
  ...
  // Per-asset tree state. Each asset has its own filledSubtrees, root history,
  // current root index, next leaf index, and an initialized flag.
  struct Tree {
  ...
  /// @dev Initialize a fresh tree for `assetId`. Idempotent-guarded by the caller.
  function _initTree(uint256 assetId) internal {
    Tree storage t = trees[assetId];
    require(!t.initialized, "tree already initialized");
    for (uint32 i = 0; i < levels; i++) {
      t.filledSubtrees[i] = zeros(i);
    }
    t.roots[0] = zeros(levels);
    t.initialized = true;
  }

  function isTreeInitialized(uint256 assetId) public view returns (bool) {
    return trees[assetId].initialized;
  }
  ...
  */
  ...
  // Insert a pair of leaves into asset `assetId`'s tree.
  function _insert(uint256 assetId, bytes32 _leaf1, bytes32 _leaf2) internal returns (uint32 index) {
    Tree storage t = trees[assetId];
  ...
  /**
    @dev Whether the root is present in `assetId`'s root history
  */
  function isKnownRoot(uint256 assetId, bytes32 _root) public view returns (bool) {
    if (_root == 0) {
      return false;
    }
    Tree storage t = trees[assetId];
    ...
    uint32 _currentRootIndex = t.currentRootIndex;
  ...
  /**
    @dev Returns the last root for `assetId`
  */
  function getLastRoot(uint256 assetId) public view returns (bytes32) {
    Tree storage t = trees[assetId];
    return t.roots[t.currentRootIndex];
  }

  /// @dev current next-leaf index for `assetId`
  function nextIndex(uint256 assetId) public view returns (uint32) {
    return trees[assetId].nextIndex;
  }

  /// @dev current filled subtree for a given level for `assetId` (debug/testing)
  function filledSubtrees(uint256 assetId, uint256 level) public view returns (bytes32) {
    return trees[assetId].filledSubtrees[level];
  }
